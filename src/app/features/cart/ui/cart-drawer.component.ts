...
@Component({
  ...
  template: `
    @if (cartService.isOpen()) {
      ...
      <div data-testid="cart-drawer" class="cart-drawer shadow-lg d-flex flex-column" tabIndex="-1">
        <!-- Header -->
        <div class="cart-header p-3 border-bottom border-purple-glow d-flex align-items-center justify-content-between">
        ...
        <div class="cart-body p-3 flex-grow-1 overflow-y-auto">
          @if (cartService.isEmpty()) {
            <div data-testid="empty-cart-message" class="empty-cart text-center py-5">
              <i class="bi bi-cart-x text-cyan fs-1 mb-3 d-block brand-icon"></i>
              <p class="text-light-purple fs-5 mb-0 fw-medium">Tu carrito está vacío</p>
            </div>
          } @else {
            <div class="cart-items-list d-flex flex-column gap-3">
              @for (item of cartService.cartItems(); track item.product.id) {
                <div data-testid="cart-item" class="cart-item glass-item p-3 rounded-3 border border-purple-glow d-flex align-items-center gap-3">
                  <img
                  ...
                  <div class="flex-grow-1 min-w-0">
                    <h6 data-testid="cart-item-title" class="cart-item-title text-light fw-semibold mb-1 text-truncate" [title]="item.product.title || item.product.name">
                      {{ item.product.title || item.product.name }}
                    </h6>
                    <div data-testid="cart-item-price" class="cart-item-price text-neon-cyan fw-bold">
                  ...
                  <div class="d-flex align-items-center gap-1">
                    ...
                    <span data-testid="item-quantity" class="item-quantity px-2 text-light fw-bold fs-6">
                      {{ item.quantity }}
                    </span>
                    <button
                      type="button"
        ...
        <!-- Footer -->
        <div class="cart-footer p-3 border-top border-purple-glow bg-dark-purple">
          <div class="d-flex justify-content-between align-items-center mb-3">
            <span class="text-light fw-semibold fs-5">Total:</span>
            <span data-testid="cart-total" class="cart-total fs-3 fw-bold text-neon-cyan">
  ...
  styles: [`
    ...
    .cart-drawer {
      ...
      z-index: 1050;
      box-shadow: -10px 0 35px rgba(0, 0, 0, 0.6), 0 0 25px rgba(0, 229, 255, 0.2);
    ...
    .glass-item:hover {
      ...
      box-shadow: 0 4px 15px rgba(0, 0, 0, 0.4), 0 0 12px rgba(0, 229, 255, 0.15);
    }
    .cart-item-img {
      width: 52px;
      height: 52px;
...
export class CartDrawerComponent {
  ...
  handleImageError(event: Event): void {
    ...
    img.src = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 50 50"><rect width="50" height="50" fill="%231a0c36"/><text x="25" y="30" font-family="sans-serif" font-size="20" fill="%2300e5ff" text-anchor="middle">⚡</text></svg>';
  }
}
