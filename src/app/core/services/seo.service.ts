...
export class SeoService {
  ...
  private defaultDescription = 'Find the best cloud development tools and services in the CloudForge Marketplace.';
  private defaultKeywords = 'cloud, development, tools, marketplace, cloudforge';
  ...
  updateSeo(config: SeoConfig): void {
    const title = config.title ? `${config.title} | CloudForge` : this.defaultTitle;
    this.titleService.setTitle(title);

    this.metaService.updateTag({ name: 'description', content: config.description || this.defaultDescription });
    this.metaService.updateTag({ name: 'keywords', content: config.keywords || this.defaultKeywords });
    
    // Open Graph
    this.metaService.updateTag({ property: 'og:title', content: title });
    this.metaService.updateTag({ property: 'og:description', content: config.description || this.defaultDescription });
    if (config.image) {
    ...
    this.metaService.updateTag({ name: 'twitter:description', content: config.description || this.defaultDescription });
    if (config.image) {
