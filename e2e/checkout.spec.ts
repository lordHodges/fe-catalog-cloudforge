...
test.describe("Tier 4: Checkout Critical Flow & Real-World User Journey", () => {
  ...
  test("TC-CHK-01: Should render checkout form with all customer input fields", async ({
    page,
  }) => {
    ...
    const checkoutBtn = page
      .locator(
        '[data-testid="proceed-to-checkout-btn"], button:has-text("Checkout"), a[href="/checkout"]',
      )
      .first();
    ...
    const nameInput = page.locator(
      '[data-testid="customer-name"], input[name="name"]',
    );
    const emailInput = page.locator(
      '[data-testid="customer-email"], input[name="email"], input[type="email"]',
    );
    const addressInput = page.locator(
      '[data-testid="customer-address"], input[name="address"]',
    );
    const cityInput = page.locator(
      '[data-testid="customer-city"], input[name="city"]',
    );
    const zipCodeInput = page.locator(
      '[data-testid="customer-zip"], input[name="zipCode"], input[name="zip"]',
    );
    const submitBtn = page.locator(
      '[data-testid="submit-order-btn"], button[type="submit"], button:has-text("Realizar pedido"), button:has-text("Pay")',
    );
  ...
  test("TC-CHK-02: Should execute complete user journey (Catalog -> Cart -> Checkout -> Order Submission)", async ({
    page,
  }) => {
    ...
    const cartToggleBtn = page.locator(
      '[data-testid="cart-toggle-btn"], button:has-text("Carrito"), button:has-text("Cart"), .cart-icon-btn',
    );
    await cartToggleBtn.click();

    const cartDrawer = page.locator(
      '[data-testid="cart-drawer"], .cart-drawer, .offcanvas, .modal-cart',
    );
    ...
    const checkoutNavBtn = page
      .locator(
        '[data-testid="proceed-to-checkout-btn"], button:has-text("Checkout"), button:has-text("Ir al Checkout"), a[href="/checkout"]',
      )
      .first();
    ...
    const confirmationScreen = page.locator(
      '[data-testid="order-success"], [data-testid="order-confirmation"], .order-success, h2:has-text("Gracias"), h2:has-text("Order Confirmed")',
    );
    ...
    if (checkoutPayload) {
      ...
      expect(checkoutPayload.customer).toHaveProperty(
        "email",
        "alex@example.com",
      );
  ...
  test("TC-CHK-03: Should clear cart signal state after successful checkout submission", async ({
    page,
  }) => {
    ...
    const submitBtn = page.locator(
      '[data-testid="submit-order-btn"], button[type="submit"]',
    );
    await submitBtn.click();

    // Wait for order confirmation view
    const confirmationScreen = page.locator(
      '[data-testid="order-success"], [data-testid="order-confirmation"]',
    );
    await expect(confirmationScreen.first()).toBeVisible();

    // Check cart badge resets to 0 or hidden
    const cartBadge = page.locator(
      '[data-testid="cart-count-badge"], .cart-badge',
    );
