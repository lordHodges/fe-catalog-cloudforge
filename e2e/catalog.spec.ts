...
test.describe('Tier 1: Catalog Feature Coverage', () => {
  ...
  test('TC-CAT-01: Should load catalog page and display product grid with required elements', async ({ page }) => {
    ...
    const productName = firstCard.locator('[data-testid="product-name"], .product-title, h3, h5');
    const productPrice = firstCard.locator('[data-testid="product-price"], .product-price, .price');
    const addBtn = firstCard.locator('[data-testid="add-to-cart-btn"], button:has-text("Add to Cart"), button:has-text("Agregar")');
  ...
  test('TC-CAT-02: Should update header cart badge when adding product to cart (Signals state update)', async ({ page }) => {
    const cartBadge = page.locator('[data-testid="cart-count-badge"], .cart-badge, [data-testid="cart-badge"]');
    
    // Initial badge count check (0 or hidden)
    const initialCount = await cartBadge.isVisible() ? await cartBadge.innerText() : '0';
    ...
    expect(parseInt(newCount, 10)).toBeGreaterThan(parseInt(initialCount || '0', 10));
  });

  test('TC-CAT-03: Should filter products by category when category selection changes', async ({ page }) => {
    const categoryFilter = page.locator('[data-testid="category-filter"], select[name="category"], .category-filter button');
  ...
  test('TC-CAT-04: Should display stock status indicators for available products', async ({ page }) => {
    const stockIndicator = page.locator('[data-testid="product-stock"], .stock-badge, .badge').first();
